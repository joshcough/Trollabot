...
trait ParserCombinators extends ScalaEnrichment {
  ...
  trait Parser[+T] extends (List[String] => ParseResult[T]) { self =>
    ...
    /**
      ...
      */
    def or[U](p2: => Parser[U]): Parser[Either[T, U]] =
  ...
  /**
    ...
    */
  def anyStringAs(name: String): Parser[String] =
    new Parser[String] {
      def apply(args: List[String]): ParseResult[String] =
        args match {
  ...
  val eof: Parser[Unit] = new Parser[Unit] {
    def apply(args: List[String]): ParseResult[Unit] =
      args match {
  ...
  val newFile: Parser[File] = attempt("new-file") { s =>
    val f = new File(s); f.createNewFile; f
  }
