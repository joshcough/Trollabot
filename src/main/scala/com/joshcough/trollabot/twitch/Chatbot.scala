...
object Chatbot {
  ...
  def streamFromDb(ircConfig: IrcConfig, xa: Transactor[IO])(implicit
    ...
    Chatbot[IO](ircConfig).stream(Api.db, xa)
...
case class Chatbot[F[_]: Async: Network](ircConfig: IrcConfig) {
  ...
  def stream(api: Api[ConnectionIO], xa: Transactor[F])(implicit
      L: LogIOStrict[F]
    ...
    Irc(ircConfig, joinMessages)(cm =>
      CommandRunner(Commands.commands).processMessage(cm, api).transact(xa).flatMap {
