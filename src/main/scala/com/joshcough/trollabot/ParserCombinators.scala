...
trait ParserCombinators extends ScalaEnrichment {
  ...
  object Parser {
    def apply[T](t: T): Parser[T] = new Parser[T] {
      def apply(args: List[String]): ParseResult[T] = Success(t, args)
      def describe: String = t.toString
    }
  }

  trait Parser[+T] extends (List[String] => ParseResult[T]) { self =>
    def apply(args: List[String]): ParseResult[T]
    def apply(args: String): ParseResult[T] = this.apply(args.split(" ").toList.filter(_.nonEmpty))
    def describe: String
     ...
     */
    def map[U](f: T => U): Parser[U] = flatMap(t => Parser(f(t)))
     ...
     */
    def or[U](p2: => Parser[U]): Parser[Either[T, U]] = new Parser[Either[T, U]] {
     ...
     */
    def |[U >: T] (p2: => Parser[U]): Parser[U] = or(p2) ^^ (_.fold(identity, identity))
     ...
     */
    def ? : Parser[Option[T]] = new Parser[Option[T]] {
      def apply(args: List[String]): ParseResult[Option[T]] =
        self(args).fold(_ => Success(None: Option[T], args))((t, rest) => Success(Some(t), rest))
    ...
    def ~>[U](p2:Parser[U]): Parser[U] =
      (for (_ <- this; u <- p2) yield u) named (self.describe + " ~> " + p2.describe)
    ...
    def <~[U](p2:Parser[U]): Parser[T] =
      (for (t <- this; _ <- p2) yield t) named (self.describe + " <~ " + p2.describe)
     ...
     */
    def named(name: => String): Parser[T] = new Parser[T] {
      def apply(args: List[String]): ParseResult[T] = self(args)
   ...
   */
  def success[T](t: T): Parser[T] = Parser(t)

  /**
   * Create a parser that always fails with the given error message.
   */
  def failure[T](message: String): Parser[T] = new Parser[T] {
    def apply(args: List[String]): ParseResult[T] = Failure(message)
    def describe: String = message
  }
  ...
  def anyStringAs(name: String): Parser[String] = new Parser[String] {
    def apply(args: List[String]): ParseResult[String] = args match {
  ...
  val eof: Parser[Unit] = new Parser[Unit] {
    def apply(args: List[String]): ParseResult[Unit] = args match {
   ...
   * Alias for eof
   */
  val noArguments: Parser[Unit] = eof

  /**
   * Alias for eof
   */
  val nothing: Parser[Unit] = eof

  /**
   * Alias for eof
   */
  val empty: Parser[Unit] = eof
  ...
  val remainingArgs: Parser[List[String]] = new Parser[List[String]] {
    def apply(args: List[String]): ParseResult[List[String]] = Success(args, Nil)
